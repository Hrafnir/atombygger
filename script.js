...
// === SEKSJON: Kjemilab (Interaktiv Sandkasse med Klynge-Graf & Frie Elektroner) ===
let labMode = 'share'; 
let labAtoms = []; 
let freeElectrons = []; // Holder oversikt over løse elektroner
let atomIdCounter = 0;
let labHistory = []; 
let currentZoom = 1.0; 
...
// Nye DOM-elementer for elektroner & søppelbøtte
const btnSpawnElectron = document.getElementById('btn-spawn-electron');
const electronTrashZone = document.getElementById('electron-trash-zone');
...
btnLabFullscreen.addEventListener('click', () => {
    if (!document.fullscreenElement && !document.webkitFullscreenElement) {
        if (labExportArea.requestFullscreen) labExportArea.requestFullscreen();
        else if (labExportArea.webkitRequestFullscreen) labExportArea.webkitRequestFullscreen();
...
function saveLabState() {
    const stateStr = JSON.stringify({
        atoms: labAtoms.map(a => ({
            ...
            sharedBonds: a.sharedBonds.map(e => ({ id: e.id, sharedWithId: e.sharedWithId }))
        })),
        frees: freeElectrons.map(e => ({ id: e.id, x: e.x, y: e.y }))
    });
...
btnLabUndo.addEventListener('click', () => {
    ...
    hardClearLab(false); 
    prevState.atoms.forEach(savedAtom => rebuildAtomFromState(savedAtom));
    prevState.frees.forEach(savedFree => {
        const eObj = createFreeElectronObject(savedFree.x, savedFree.y);
        eObj.id = savedFree.id; // Bevar ID
        renderFreeElectronDOM(eObj);
    });
...
function hardClearLab(clearHistory = false) {
    ...
    labCanvasInner.querySelectorAll('.free-electron').forEach(e => e.remove());
...
function getLogicalCoords(clientX, clientY) {
    ...
    return { x: 2000 + offsetX, y: 2000 + offsetY };
}
...
// === FRIE ELEKTRONER (Seksjon 3 logikk) ===
btnSpawnElectron.addEventListener('click', () => {
    saveLabState();
    labPlaceholder.style.display = 'none';
    ...
    const eObj = createFreeElectronObject(
        2000 + (Math.random() * 100 - 50),
        2000 + (Math.random() * 100 - 50)
    );
    renderFreeElectronDOM(eObj);
});

function createFreeElectronObject(x, y) {
    return {
        id: 'free_e_' + atomIdCounter++,
        x: x,
        y: y,
        elementRef: null
    };
...
function renderFreeElectronDOM(eObj) {
    const eEl = document.createElement('div');
    eEl.className = 'free-electron';
    eEl.id = eObj.id;
    eEl.style.touchAction = 'none';
    eEl.style.left = `${eObj.x}px`;
    eEl.style.top = `${eObj.y}px`;
    
    eObj.elementRef = eEl;
    labCanvasInner.appendChild(eEl);
    freeElectrons.push(eObj);
    
    makeFreeElectronDraggable(eObj);
}

function isOverTrashZone(clientX, clientY) {
    const rect = electronTrashZone.getBoundingClientRect();
    return (clientX >= rect.left && clientX <= rect.right &&
            clientY >= rect.top && clientY <= rect.bottom);
}

function executeTrash(eObj, sourceAtom) {
    // Fjerner fra atomet
    sourceAtom.electrons = sourceAtom.electrons.filter(e => e.id !== eObj.id);
    sourceAtom.charge += 1; // Atomet blir mer positivt når det mister negativ ladning
    
    eObj.elementRef.remove();
    recalculateElectronAngles(sourceAtom);
    updateChargeVisuals(sourceAtom);
}

function makeFreeElectronDraggable(eObj) {
    ...
    function drag(clientX, clientY) {
        ...
        if (isOverTrashZone(clientX, clientY)) electronTrashZone.classList.add('drag-over');
        else electronTrashZone.classList.remove('drag-over');
    ...
    function endDrag(clientX, clientY) {
        ...
        electronTrashZone.classList.remove('drag-over');

        if (isOverTrashZone(clientX, clientY)) {
            el.remove();
            freeElectrons = freeElectrons.filter(e => e.id !== eObj.id);
            updateLabAnalysis();
            return;
        }
        ...
        if (dropTarget) {
            ...
            const newE = {
                id: eObj.id,
                ownerId: dropTarget.id,
                isShared: false,
                sharedWithId: null,
                angle: 0,
                elementRef: null
            };
            dropTarget.electrons.push(newE);
            dropTarget.charge -= 1; // Atomet blir mer negativt
            
            // Tegn det nye elektronet i banen til atomet
            const radius = 45;
            const newEl = document.createElement('div');
            newEl.className = 'valence-electron';
            newEl.id = newE.id;
            newEl.style.touchAction = 'none';
            newE.elementRef = newEl;
            dropTarget.elementRef.appendChild(newEl);
            makeElectronDraggable(newE, dropTarget);
            
            recalculateElectronAngles(dropTarget);
            updateChargeVisuals(dropTarget);
        }
        updateLabAnalysis();
    }

    el.addEventListener('mousedown', (e) => { e.stopPropagation(); startDrag(); });
    document.addEventListener('mousemove', (e) => drag(e.clientX, e.clientY));
    document.addEventListener('mouseup', (e) => endDrag(e.clientX, e.clientY));

    el.addEventListener('touchstart', (e) => { e.stopPropagation(); startDrag(); }, {passive: false});
    document.addEventListener('touchmove', (e) => { if (!isDragging) return; drag(e.touches[0].clientX, e.touches[0].clientY); }, {passive: false});
    document.addEventListener('touchend', (e) => { if (!isDragging) return; endDrag(e.changedTouches[0].clientX, e.changedTouches[0].clientY); });
}
...
function makeElectronDraggable(eObj, sourceAtomObj) {
    ...
    function endDrag(clientX, clientY) {
        ...
        if (isOverTrashZone(clientX, clientY)) {
            executeTrash(eObj, sourceAtomObj);
            updateLabAnalysis();
            return;
        }
...
// === GRAF-ALGORITME FOR FORMLER ===
function getMoleculeClusters() {
    let visited = new Set();
    let clusters = [];

    // Finner ut om to atomer er ionisk bundet (motsatt ladning og nærme hverandre)
    function isIonicBond(a, b) {
        if (a.charge === 0 && b.charge === 0) return false;
        if (a.charge * b.charge >= 0) return false; // Må være en pluss og en minus
        let dist = Math.hypot(a.x - b.x, a.y - b.y);
        return dist < 140; // Gitter-avstand
    }

    for (let i = 0; i < labAtoms.length; i++) {
        if (!visited.has(i)) {
            let cluster = [];
            let queue = [i];
            visited.add(i);
            ...
            while (queue.length > 0) {
                ...
                let currAtom = labAtoms[currIdx];
                cluster.push(currAtom);

                for (let j = 0; j < labAtoms.length; j++) {
                    if (!visited.has(j)) {
                        let otherAtom = labAtoms[j];
                        
                        let covalent = currAtom.sharedBonds.some(b => b.sharedWithId === otherAtom.id) ||
                                       otherAtom.sharedBonds.some(b => b.sharedWithId === currAtom.id);
                        
                        let ionic = isIonicBond(currAtom, otherAtom);
...
function buildClusterFormula(cluster) {
    const counts = {};
    let netCharge = 0;
    cluster.forEach(a => {
    ...
    let remainingSymbols = Object.keys(counts).sort((a, b) => {
        const atomA = cluster.find(atom => atom.symbol === a);
        const atomB = cluster.find(atom => atom.symbol === b);
    ...
    if (netCharge !== 0) {
        ...
        let num = Math.abs(netCharge);
        let numStr = num > 1 ? getSuperscript(num) : '';
        formula += numStr + sign;
    }
...
function updateLabAnalysis() {
    if (labAtoms.length === 0 && freeElectrons.length === 0) {
    ...
    labAtoms.forEach(atom => {
        ...
        if (effectiveValence !== targetOctet) {
            if (atom.charge > 0 && nativeOwned === 0) {
                // Stabil kation (tomt ytterskall betyr at underliggende er fullt)
            } else {
                allHappy = false;
            }
        }
    ...
    // Grupperer i molekyler og ioner for å regne ut 2FeO etc.
    let clusters = getMoleculeClusters();
    let formulaCounts = {};
    
    clusters.forEach(cluster => {
        let f = buildClusterFormula(cluster);
        formulaCounts[f] = (formulaCounts[f] || 0) + 1;
    });
    
    let finalFormulaParts = [];
    Object.keys(formulaCounts).sort().forEach(f => {
        let count = formulaCounts[f];
        if (count > 1) {
            finalFormulaParts.push(count + f);
        } else {
            finalFormulaParts.push(f);
        }
    });

    if (freeElectrons.length > 0) {
        let eStr = freeElectrons.length > 1 ? freeElectrons.length + 'e⁻' : 'e⁻';
        finalFormulaParts.push(eStr);
    }
    
    labResultFormula.textContent = finalFormulaParts.join(' + ');

    if (allHappy && freeElectrons.length === 0) {
        ...
        labResultExplanation.textContent = 'Dra elektroner mellom atomene for å dele (molekyl) eller stjele (ioner) slik at alle får 8 elektroner i ytterste skall (2 for Hydrogen). Bruk evt søppelbøtten for å fjerne elektroner for å lage positive ioner.';
    }
}
